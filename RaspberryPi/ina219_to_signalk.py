...
sensor_labels = {
    0: ("INA4", "sensors.startboard.oilpressure.voltage"),
    1: ("INA3", "sensors.port.oilpressure.voltage"),
    2: ("INA2", "sensors.starboard.watertemp.voltage"),
    3: ("INA1", "sensors.port.watertemp.voltage")
}

def send_to_signalk(ws, sensor_id, voltage, current, power):
    label, path = sensor_labels[sensor_id]
    source_name = f"{label}-{path.split('.')[1].capitalize()}-{path.split('.')[2].capitalize()}"
    data = {
        "updates": [
            {
                "source": {
...
if __name__ == '__main__':
    ...
    ws.connect(SIGNALK_WS, header=[
        f"Authorization: Bearer {SIGNALK_TOKEN}",
        "X-Client-Id: INA219-Monitor"
    ])
